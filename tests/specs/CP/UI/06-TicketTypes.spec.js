...
test.describe('Ticket types', (page) => {
    ...
    // })
        test('should show all UI elements', async ({ page }) => {
            ...
            await page.waitForSelector('text=New Ticket Type');
        ...
        test.describe('New Type', () => {
            test('should test all fields for required', async ({ page }) => {
                ...
                await page.waitForSelector('text=New Ticket Type');
                await ticketTypesCP.btnNewTicketType.click();
                await ticketTypesCP.inputName.fill('123');
                await ticketTypesCP.inputPosition.click();
                await ticketTypesCP.inputName.fill('');
                await ticketTypesCP.inputPosition.click();
            ...
            test.describe('Inputs', () => {
                test.describe('Name', () => {
                    test('name field uniqueness constraint', async ({ page }) => {
                        ...
                        await ticketTypesCP.inputMinimumQty.fill('1')
                        await ticketTypesCP.selectVat.selectOption('16.00');
                        await ticketTypesCP.selectTicketCategory.selectOption('alreadyExist');
                        await ticketTypesCP.btnSubmit.click();
                        await expect.soft(ticketTypesCP.overlayCreateError).toBeVisible({timeout: 10000});
                ...
                test.describe('Minimum qauntity', () => {
                    test.skip('shouldnt create a ticket type with invalid minimum quantity', async ({ page }) => {
                        const ticketTypesCP = new TicketTypesCP(page, lang);
                        const id30char = uuidv4();
                        const name = id30char.substring(0, 20);
                        await page.waitForSelector('text=New Ticket Type');
                        ...
                        await ticketTypesCP.inputPrice.fill('100');
                        await ticketTypesCP.inputPosition.fill('100');
                        await ticketTypesCP.inputMinimumQty.fill('-')
                        await ticketTypesCP.selectVat.selectOption('16.00');
                        await ticketTypesCP.selectTicketCategory.selectOption('alreadyExist');
                        await ticketTypesCP.btnSubmit.click();
                        await expect(ticketTypesCP.overlayCreateError).toBeVisible({timeout: 10000});
                    ...
                    test('shouldnt create a ticket type with negative minimum quantity', async ({ page }) => {
                        const ticketTypesCP = new TicketTypesCP(page, lang);
                        const id30char = uuidv4();
                        const name = id30char.substring(0, 20);
                        await page.waitForSelector('text=New Ticket Type');
                        ...
                        await ticketTypesCP.btnSubmit.click();
                        await expect(ticketTypesCP.overlayCreateError).toBeVisible({timeout: 10000});
                        await expect(ticketTypesCP.errorLabelminQuantityMin).toBeVisible();
                    });
                    test('shouldnt create a ticke type with minimum quantity above 100', async ({ page }) => {
                        const ticketTypesCP = new TicketTypesCP(page, lang);
                        const id30char = uuidv4();
                        const name = id30char.substring(0, 20);
                        await page.waitForSelector('text=New Ticket Type');
                        ...
                        await ticketTypesCP.inputPrice.fill('100');
                        await ticketTypesCP.inputPosition.fill('100');
                        await ticketTypesCP.inputMinimumQty.fill('101')
                        await ticketTypesCP.selectVat.selectOption('16.00');
                        await ticketTypesCP.selectTicketCategory.selectOption('alreadyExist');
                        await ticketTypesCP.btnSubmit.click();
                        await expect(ticketTypesCP.overlayCreateError).toBeVisible({timeout: 10000});
                        await expect(ticketTypesCP.errorLabelminQuantityMax).toBeVisible();
                    });
                });
                test.describe('Position', () => {
                    test.skip('shouldnt create a ticket type with invalid position', async ({ page }) => {
                        ...
                        await ticketTypesCP.inputName.fill(name);
                        await ticketTypesCP.inputPrice.fill('100');
                        await ticketTypesCP.inputPosition.fill('-');
                        await ticketTypesCP.inputMinimumQty.fill('1')
                        await ticketTypesCP.selectVat.selectOption('16.00');
                        await ticketTypesCP.selectTicketCategory.selectOption('alreadyExist');
                        await ticketTypesCP.btnSubmit.click();
        ...
        test.describe('Edit Ticket Type', () => {
            ...
            test('all fields are prefilled when editing a ticket type', async ({page}) => {
                const ticketTypesCP = new TicketTypesCP(page, lang);
                await page.waitForSelector('text=New Ticket Type');
                await page.getByText('forEdit').first().click();
                await page.waitForSelector('text=Edit Ticket Type');
                await expect.soft(ticketTypesCP.inputName).toHaveValue('forEdit');
                await expect.soft(ticketTypesCP.inputDescription).toHaveValue("don't remove - used for AUTOMATION");
                await expect.soft(ticketTypesCP.inputMinimumQty).toHaveValue('1');
                await expect.soft(ticketTypesCP.inputPosition).toHaveValue('0');
                await expect.soft(ticketTypesCP.selectTicketCategory).toContainText('forEdit');
                await expect.soft(ticketTypesCP.inputPrice).toHaveValue('10.00');
                await expect.soft(ticketTypesCP.selectVat).toHaveText('16.00');
            });

            test('name field uniqueness constraint', async ({ page }) => {
                const ticketTypesCP = new TicketTypesCP(page, lang);
                await page.waitForSelector('text=New Ticket Type');
                await page.getByText('alreadyExist2').first().click();
                await page.waitForSelector('text=Edit Ticket Type');
                await ticketTypesCP.inputName.fill('alreadyExist');
            ...
            test.describe('Function', (page) => {
                test('should update a ticket type - all fields', async ({ page }) => {
                    ...
                    await page.getByText(name).first().click();
                    await page.waitForSelector('text=Edit Ticket Type');
                    await ticketTypesCP.inputName.fill(nameUpdated);
                    ...
                    await page.waitForSelector('text=New Ticket Type');
                    await expect(page.locator('text=' + nameUpdated)).toBeVisible();

                    await page.getByText(nameUpdated).first().click();
                    await ticketTypesCP.inputName.fill('updateName');
                    await ticketTypesCP.inputPrice.fill('200.00');
                    await ticketTypesCP.inputPosition.fill('10');
                    // await ticketTypesCP.selectIcon.selectOption('Cash');
                    // await ticketTypesCP.selectVat.selectOption('20.00');
                    await ticketTypesCP.selectTicketCategory.selectOption('Bus');
                    await ticketTypesCP.btnUpdate.click();
        ...
        test.describe.skip('Delete Ticket Type', (page) => {
