const LoginScreen = require('../../screenObjects/English/DI/login.screen');
const loginDI = async (page, user) => {
    const loginScreen = new LoginScreen(page);
    await page.goto('https://seashell-app-pkyfy.ondigitalocean.app/');
    await page.getByRole('heading', { name: 'WELCOME ABOARD' }).waitFor({state:"visible"});
...
const nextWeek = async (page, booking) => {
    await page.locator('div').filter({ hasText: /^PRIVÉ $/ }).getByRole('button', {name:'-'}).nth(1).click();
}
...
export {loginDI, pickNotifClub, createBooking, deleteBooking, deleteEvent, inviteBooking, inviteBusiness, createNotifyEvent, acceptBooking, rejectBooking, standbyBookingRequest, addCourse, removeCourse, rejectDoB, acceptDoB, blastGroup, removeBlastGroup, blastAll, blastDancers, sendMessage, rejectFromAccepted };
